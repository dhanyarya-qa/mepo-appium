...
def _go_home(driver):
    """Navigate back to home screen safely without randomly pressing back and exiting app."""
    # First, let's check if we are already somewhere on home (Open trip card is visible, or Itinerary)
    for _ in range(2):
        home_markers = driver.find_elements(AppiumBy.XPATH,
            "//*[contains(@content-desc, 'Welcome,') or contains(@content-desc, 'Open Trip') or contains(@content-desc, 'Itinerary')]")
        if home_markers:
            return True
        driver.back()
        time.sleep(2)
